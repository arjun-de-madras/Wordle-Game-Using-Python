    ...
    with open(r'scores.csv') as f:
        ...
        for i in r:
            if i[0]==userid:
                ...
                l.append(i)
    with open(r'scores.csv','w', newline='') as f:
        f.seek(0)
        w=csv.writer(f)
        w.writerows(l)
...
def on_login_submit():
    ...
    with open(r'userdetails.csv', ) as f:
        r=csv.reader(f)
...
def on_signup_submit():
    ...
    if confirm==pwd and len(pwd)!=0 and not is_userid_exists:
        cur_user=userid
        with open(r'userdetails.csv', 'a', newline='') as f, open(r'scores.csv','a', newline='') as f1:
            w=csv.writer(f)
            w1=csv.writer(f1)
